...
const FAQs = props => {
  ...
  const handleSubmitQuestion = () => {
    ...
    let arr =
      title.length > 0
        ? { question: content, title: title, helpType: selected._id }
        : { question: content, helpType: selected._id };
    ...
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
      <View style={{ flex: 1 }}>
        ...
        <Text
          style={[
            Fonts.style.normal,
            { marginTop: 34, textAlign: 'right', paddingRight: 30 },
          ]}>
          {' '}
          {'أهلا بك عزيزي , كيف يمكننا مساعدتك ؟'}{' '}
        </Text>

        <InputButton
          width={150}
          InputPlaceHolder={
            selected && selected.userTypeName
              ? selected.userTypeName
              : 'نوع المساعدة'
          }
          containerStyle={{ marginTop: 37 }}
          onPress={() => setRadioButtonModal(true)}
        />
        ...
        <Input
          ...
          InputPlaceHolder={'عنوان الموضوع'}
        />
        ...
        <Input
          ...
          inputValue={content}
          containerStyle={{ marginTop: 37, paddingTop: 15, height: 100 }}
          InputPlaceHolder={' الموضوع'}
          InputStyle={{}}
        />
        ...
        {radioButtonModal && (
          <RadioButtonModal
            title={'نوع المساعدة'}
            onPress={item => {
              setRadioButtonModal(false);
              setSelected(item);
            }}
            selectedOption={selected}
            doAnimation={true}
            data={[
              { _id: 0, userTypeName: 'اقتراح' },
              { _id: 1, userTypeName: 'مشكلة' },
              { _id: 2, userTypeName: 'رسالة' },
            ]}
            isVisible={radioButtonModal}
          />
        )}
        {showErrorMessage && (
          <ErroAlert
