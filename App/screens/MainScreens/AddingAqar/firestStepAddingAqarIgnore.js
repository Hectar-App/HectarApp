    ...
    return (
      <View style={{ flex: 1 }}>
          ...
          <TouchableWithoutFeedback>
            <View>
              <MainTypes
                ...
                doAnimation={true}
                statusPress={i => this.statusPress(i)}
                selectedType={this.state.selectedStatus}
              />

              <Text
                style={[
                  Fonts.style.mainTitle,
                  { marginTop: 40, alignSelf: 'flex-end', marginEnd: 16 },
                ]}>
                {' '}
                أختر نوع العقار{' '}
              </Text>
              ...
              <RadioButton
                selectedPourpose={i => this.setState({ selectedPurpose: i })}
              ...
              <Input
                ...
                withDesc
                desc={'ريال سعودي'}
                InputPlaceHolder={'سعر المتر '}
                doAnimation={true}
              />

              <InputButton
                onPress={() => this.setState({ mapModalVisabl: true })}
                InputPlaceHolder={
        ...
        {this.state.mapModalVisabl && (
          <MapModal
            selectedLocation={x =>
              this.setState({ mapModalVisabl: false, selectedLocation: x })
            }
          />
        )}
