...
import ConfirmatioDigits from '../../Component/ConfirmatioDigits'
...
class RegistrationConfirmationPhone extends React.Component {
    ...
    handleVerifyCode = (code) => {
        ...
        if( this.state.fromForget ){
            this.props.forgetPasswordVerfyNumber(this.convertNumbers2English(this.state.phoneNumber), this.convertNumbers2English(code))
        }
        else{
            this.props.verifyPhone(this.convertNumbers2English(this.state.phoneNumber), this.convertNumbers2English(code))
        }
    }
    ...
    componentWillReceiveProps ( nextProps ) {
        ...
        if( this.props.verifyNumberSuccess !== nextProps.verifyNumberSuccess ){
            ...
            if( nextProps.verifyNumberSuccess.message){
                this.setState({showErrorMessage: true, errorMessage: nextProps.verifyNumberSuccess.message})
                // alert(nextProps.verifyNumberSuccess.message)
            }else{
                this.props.navigation.navigate('RegistrationUserInfo', {phone: this.state.phoneNumber, code: this.state.code})
                // alert('success')
            }
        }

        if( this.props.forgetPasswordVerfyNumberError !== nextProps.forgetPasswordVerfyNumberError ){
            if( nextProps.forgetPasswordVerfyNumberError.e ){
                this.setState({showErrorMessage: true, errorMessage: nextProps.forgetPasswordVerfyNumberError.e.message})
                // alert( nextProps.forgetPasswordVerfyNumberError.e.message )
            }
        }

        if( this.props.forgetPasswordVerfyNumberSucess !== nextProps.forgetPasswordVerfyNumberSucess ){
            this.props.navigation.navigate('ForgetPasswordResetPass', {phone: this.state.phoneNumber, code: this.state.code})
        }

        if( nextProps.checkNuberSucess !== this.props.checkNuberSucess ){
            console.log('Hello Sucess', nextProps.checkNuberSucess)
            this.setState({ reSendLoading: false, timer: true, time: 2, code: nextProps.checkNuberSucess.confirmationCode })
        ...
        if( nextProps.checkNuberError !== this.props.checkNuberError ){
            ...
            if(nextProps.checkNuberError && nextProps.checkNuberError.message)
                this.setState({showErrorMessage: true, errorMessage: nextProps.checkNuberError.message})
        }
    }
        ...
        return (
            <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
                <View style={{flex:1, backgroundColor:'#fff'}} >
                    ...
                    <Header withoutUnderLine={true} headerTitle={'التحقق من رقم الجوال'} onBackPress={()=> this.props.navigation.goBack()} />

    
                    <TitleDesc title={''} containerStyle={{marginTop: 50}} desc={`فضلا,  قم بإدخال رمز التحقق المكون من 4 ارقام  المرسل  الي جوالك ${this.state.phoneNumber} الكود ${this.state.code} `} />
                ...
                { !this.state.timer && <Animated.View
                    style={{
                 ...
                 >
                    ...
                    <LinkButton loading={this.state.reSendLoading} containerStyle={{alignSelf:'flex-end', paddingEnd: 0,  alignSelf:'center'}} buttonText={'  أرسل مرة اخرى  '} textStyle={{color: Colors.darkSeafoamGreen, fontSize: 13}} onPress={this.handleNextStep} />
                    <Text style={[Fonts.style.normal, {color: Colors.black, fontSize:13, alignSelf:'center' }]}> لم تتقلى رمز التحقق ؟ </Text>
                 </Animated.View>}
    
                 {/* <Text style={[Fonts.style.normal, {color: Colors.black, fontSize:13, alignSelf:'center' }]}>1:99</Text> */}
                {this.state.timer && <Timer style={[Fonts.style.normal, {color: Colors.black, fontSize:13, alignSelf:'center',  marginTop:40 }]} done={() => this.setState({timer: false})} type={'minute'} number={this.state.time} />}
    
                    <Button doAnimation={true} loading={this.state.loading} containerStyle={{marginTop:80}}  buttonText={'تحقق الأن'} onPress={()=> this.handleVerifyCode()} />
...
const mapDispatchToProps = dispatch => ({
    ...
    forgetPasswordVerfyNumber: (phone, code) => dispatch(UserAction.forgetPasswordVerfyNumber(phone, code)),
