...
class ForgetPasswordResetPass extends React.Component {
    ...
    componentWillReceiveProps(nextProps){
        if( nextProps.resetPasswordSucess !== this.props.resetPasswordSucess){
            ...
            if(nextProps.resetPasswordSucess.success){
                alert('تم اعادة كلمة المرور بنجاح!')
                this.props.navigation.dispatch(StackActions.reset({
                    index: 0,
                    actions: [NavigationActions.navigate({ routeName: 'LoginPage' })],
                }))
            }else if(nextProps.resetPasswordSucess.message){
                alert(nextProps.resetPasswordSucess.message)
            }
        }
    ...
    handleReset () {
        ...
        console.log('this.stat.phoneNumber', phoneNumber, code, password, cPassword )

        if( password !== cPassword  )
               return alert('كلمة المرور لا تتطابق مع تأكيد كلمة المرور')
            //    || (password || '').length < 7
        if( (password || '').length < 7  )
            return alert("يجب ان تكون كلمة المرور اكثر من ٧ خانات")
        ...
        return (
            <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
                <View style={{flex:1, backgroundColor:'#fff'}} >
                    ...
                    <TitleDesc title={'تغيير كلمة المرور'} containerStyle={{marginTop: 50}} desc={'أدخل كلمة المرور الجديدة ادناه '}/>
                    <View style={{marginTop: 110}} >
                        <Input onChangeText={(val)=> this.setState({ password: val})} value={this.state.password} width={150}  InputPlaceHolder={'كلمة المرور الجديدة'} containerStyle={{ marginTop: 0 }} />
                        <Input onChangeText={(val)=> this.setState({ cPassword: val})} value={this.state.cPassword} width={150}  InputPlaceHolder={'تأكيد كلمة المرور'} containerStyle={{ marginTop: 12 }} />
                    </View>
                    <Button loading={this.state.loading} containerStyle={{marginTop:86}}  buttonText={'إعادة كلمة المرور'} onPress={()=> this.handleReset()} />
                </View>
...
const mapDispatchToProps = dispatch => ({
    resetPassword: (phone, code, password) => dispatch(UserAction.resetPasswrd(phone, code, password)),
  });
