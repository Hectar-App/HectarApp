...
import { View, Text, TextInput, StyleSheet, Platform, Animated, TouchableOpacity, FlatList, TouchableHighlight } from 'react-native';
...
const MarkerItem = (props) => {
    ...
    let colorCheck = false
        colorCheck = (props.realEstates || []).findIndex(i => i === props.item._id) === -1 
    return (
    <View style={[styles.container, props.containerStyle]} key={props.item._id} >
         {!props.smallIcon && false  ? 
         <Marker
            tracksViewChanges={props.tracksViewChanges}
            coordinate={{
                latitude: props.item.address && props.item.address.coordinates && props.item.address.coordinates[0] ,
                longitude: props.item.address && props.item.address.coordinates && props.item.address.coordinates[1],
                latitudeDelta: 0.1,
        ...
        >
            <TouchableOpacity
                ...
                style={{
                backgroundColor: props.focusMarker === props.item._id ? Colors.darkSeafoamGreen: colorCheck? Colors.darkSlateBlue: Colors.grey,
                // width: 60,
                // height: 25,
                borderRadius:6,
                ...
                shadowRadius: 2,
                elevation: 5,
                zIndex: props.focusMarker === props.item._id ? 100000000000000000: 10,
                borderWidth: 1,
                borderColor: '#fff'
                }}
            >
                <Text style={[Fonts.style.normal,{color: Colors.white, fontSize: 11}]} >{ props.item.price && kFormatter(props.item.price) + " ريال " || 'السعر غير محدد'}  </Text>
            </TouchableOpacity>
            <View
                style={[ Platform.OS === 'ios'? {
                    ...
                    borderWidth: 5,
                    borderColor: props.focusMarker === props.item._id ? Colors.darkSeafoamGreen: colorCheck? Colors.darkSlateBlue: Colors.grey,
                ...
                { 
                    ...
                    borderLeftColor: 'transparent',
                    borderRightColor: 'transparent',
                    borderBottomColor: props.focusMarker === props.item._id ? Colors.darkSeafoamGreen: colorCheck? Colors.darkSlateBlue: Colors.grey,
                    transform: [
                        {rotate: '180deg'}
                    ]}
                ...
                (
                    ...
                    >
                        <TouchableOpacity
                            ...
                            onPress={()=>alert(1)}
                        >
                            <Text style={[Fonts.style.normal,{color: Colors.white, fontSize: 11}]} >{'شقة سكنية للبيع'}</Text>
                        </TouchableOpacity>
                        <View
                            style={{
                                position: 'absolute',
