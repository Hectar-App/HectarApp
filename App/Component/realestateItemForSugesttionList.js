...
const realEstateItem = (props) => {
    ...
    {/* 69.4 */}
       <Animated.View style={[styles.cardView]} >
            ...
            >
                 <Text style={[Fonts.style.normal, {fontSize: 12, alignSelf:'center', marginEnd: 2, color: Colors.white, }]} >{nameAr && nameAr} {status && status.nameAr && status.nameAr}</Text>
            </View>
            ...
            >
                <Image source={ imagesSmall && (imagesSmall || []).length > 0 ?  {uri: imagesSmall[0]}: Images.testCardImage} style={{width: '100%', height: '100%', borderRadius: 20}} />
            ...
            >
                ...
                >
                    <Text style={[Fonts.style.normal, {fontSize: 15, alignSelf:'center', color: Colors.darkSlateBlue, fontWeight: Platform.OS === 'android'?'400': "bold",}]} >{ price && kFormatter(price) || 'السعر غير محدد'}</Text>
                ...
                >
                    ...
                    <View style={{flexDirection: 'row-reverse', alignItems:'center', alignSelf: 'flex-end'}}>
                        <Text numberOfLines={1} style={[Fonts.style.normal, {fontSize: 11, alignSelf:'center', fontWeight: 'normal', marginStart: 3, color: '#464646', width: '90%', textAlign: 'left'}]} > {address? address: coordinates[0]+','+coordinates[1]} </Text>
