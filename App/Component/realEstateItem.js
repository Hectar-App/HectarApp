...
import { View, Text, Platform, TextInput, StyleSheet, Animated, TouchableOpacity, FlatList, TouchableHighlight, Image } from 'react-native';
...
const realEstateItem = (props) => {
    ...
    return (
    <TouchableOpacity style={[styles.container, props.containerStyle, ]} onPress={() => props.onPress(props.item)} >
    {/* 69.4 */}
    {/* {transform: [{translateX: translate}]} */}
       <Animated.View style={[styles.cardView, ]} >
            ...
            >
                 <Text style={[Fonts.style.normal, {fontSize: 12, alignSelf:'center', marginEnd: 2, color: Colors.white, }]} >
                 {/* { status && status.nameAr ? status.nameAr: "للبيع"} */}
                 {type && type.nameAr && type.nameAr} {status && status.nameAr && status.nameAr}
                 </Text>
            </View>
                    ...
                    renderItem={(item)=>{
                        return (
                            <TouchableOpacity onPress={() => props.onPress(props.item)} >
                                <FastImage
                                    style={{ width: '100%', height: '100%', borderTopRightRadius: 20, borderTopLeftRadius: 20 }}
                                    source={images && (images || []).length > 0 ?  {uri: item.item} : Images.testCardImage}
                                    onLoadEnd={() => {setLoading(false)}}
                                    onError={() => {
                                        console.log('error')
                                    }}
            ...
            >
                ...
                >
                    ...
                    >
                        {(numberOfRooms || numberOfRooms === '') && <View style={{alignItems: 'center', marginEnd: 8}} >
                        ...
                        { (numberOfBathRoom || numberOfBathRoom === '') && <View style={{alignItems: 'center', marginEnd: 8}} >
                        ...
                        {(numberOfLivingRoom || numberOfLivingRoom === '') && <View style={{alignItems: 'center', marginEnd: 8}} >
                        ...
                        {(space || space === '') && <View style={{alignItems: 'center'}} >
                    ...
                    <Text style={[Fonts.style.normal, {fontSize: 15, alignSelf:'center', color: Colors.darkSlateBlue, fontWeight: Platform.OS === 'android'?'400': "bold",}]} >{price && kFormatter(price) + " ريال "  || 'السعر غير محدد'}</Text>
                </View>
                ...
                {!props.mine ? 
                    ...
                    >
                        <View style={{flexDirection: 'row',  justifyContent:'center', alignItems:'center'}}>
                            {updatedAt &&<Text style={[Fonts.style.normal, {fontSize: 12, alignSelf:'center', marginEnd: 2}]} >{timeSince(new Date(updatedAt))}</Text>}
                            
                            <Image source={Images.watchFlagCardIcon} />
                        </View>
                        <View style={{flexDirection: 'row', alignItems:'center', justifyContent: 'flex-end' }}>
                            <Text numberOfLines={1} style={[Fonts.style.normal, {fontSize: 12, alignSelf:'flex-end', color: '#464646',  width: '75%', textAlign: 'right'}]} > {address || coordinates[0] +' ,'+coordinates[1]} </Text>
                    ...
                    >
                        { props.dismassLoading === _id ? 
                            <TouchableOpacity
                                style={{flexDirection: 'row', alignItems:'center', height: 30, borderRadius: 5, paddingHorizontal: 6, backgroundColor: active === 1? Colors.redWhite: Colors.darkSeafoamGreen}}
                            >
                                <ActivityIndicator color={Colors.white} />
                            </TouchableOpacity>
                            :
                            <TouchableOpacity onPress={props.onDismassPress} style={{flexDirection: 'row', alignItems:'center', height: 30, borderRadius: 5, paddingHorizontal: 6, backgroundColor: active === 1? Colors.redWhite: Colors.darkSeafoamGreen}}>
                                <Text style={[Fonts.style.normal, {fontSize: 12, alignSelf:'center', color: '#fff',}]} > {active === 1 ? 'اخفاء': 'تنشيط'} </Text>
                            </TouchableOpacity>
                        }
                        <View style={{flexDirection: 'row', justifyContent:'center', alignItems:'center', height: 30, borderRadius: 5, paddingHorizontal: 6, backgroundColor: active === 1? Colors.darkSeafoamGreen: Colors.redWhite}}>
                            <Text style={[Fonts.style.normal, {fontSize: 12, textAlign:'center', alignSelf:'center', color: '#fff'}]} >{active === 1 ? 'نشط': 'موقوف'}</Text>
                            {/* <Image source={Images.watchFlagCardIcon} /> */}
                        </View>
                    </View>
                }
